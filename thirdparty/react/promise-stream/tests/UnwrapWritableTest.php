...
namespace Catenis\WP\React\Tests\Promise\Stream;
...
use Catenis\WP\React\Promise\Stream;
...
class UnwrapWritableTest extends TestCase
{
    ...
    {
        $input = $this->getMockBuilder('Catenis\WP\React\Stream\WritableStreamInterface')->getMock();
    ...
    public function testWriteReturnsTrueWhenUnwrappedStreamReturnsTrueForWrite()
    {
        ...
        $promise = \Catenis\WP\React\Promise\resolve($input);
        $stream = Stream\unwrapWritable($promise);
    ...
    public function testClosingStreamWillCloseStreamFromCancellationHandler()
    {
        ...
        $promise = new \Catenis\WP\React\Promise\Promise(function () { }, function ($resolve) use ($input) {
            $resolve($input);
        });
    ...
    {
        $promise = \Catenis\WP\React\Promise\resolve(new ThroughStream());
        $stream = Stream\unwrapWritable($promise);
