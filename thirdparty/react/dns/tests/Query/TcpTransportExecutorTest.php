...
use Catenis\WP\React\Dns\Protocol\BinaryDumper;
use Catenis\WP\React\Dns\Protocol\Parser;
...
class TcpTransportExecutorTest extends TestCase
{
    ...
    public function testQueryRejectsOnCancellationWithoutClosingSocketButStartsIdleTimer()
    {
        ...
        $timer = $this->getMockBuilder('Catenis\WP\React\EventLoop\TimerInterface')->getMock();
    ...
    public function testQueryRejectsWhenServerIsNotListening()
    {
        ...
        \Catenis\WP\Clue\React\Block\sleep(0.01, $loop);
        if ($wait) {
            \Catenis\WP\Clue\React\Block\sleep(0.2, $loop);
        }
    ...
    public function testQueryResolvesIfServerSendsValidResponse()
    {
        ...
        $promise = $executor->query($query);
        $response = \Catenis\WP\Clue\React\Block\await($promise, $loop, 0.2);

        $this->assertInstanceOf('Catenis\WP\React\Dns\Model\Message', $response);
    }
