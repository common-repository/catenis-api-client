...
$socket = new \Catenis\WP\React\Socket\Server('127.0.0.1:9001', $loop);

$closeFrameChecker = new \Catenis\WP\Ratchet\RFC6455\Messaging\CloseFrameChecker;
$negotiator = new \Catenis\WP\Ratchet\RFC6455\Handshake\ServerNegotiator(new \Catenis\WP\Ratchet\RFC6455\Handshake\RequestVerifier);
...
$socket->on('connection', function (React\Socket\ConnectionInterface $connection) use ($negotiator, $closeFrameChecker, $uException) {
    ...
    $connection->on('data', function ($data) use ($connection, &$parser, &$headerComplete, &$buffer, $negotiator, $closeFrameChecker, $uException) {
        ...
        $headerComplete = true;
        $psrRequest = \Catenis\WP\GuzzleHttp\Psr7\parse_request($parts[0] . "\r\n\r\n");
        ...
        $parser = new \Catenis\WP\Ratchet\RFC6455\Messaging\MessageBuffer($closeFrameChecker,
